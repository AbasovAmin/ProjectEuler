...
#Your task has been made easy, as the encryption key consists of three lower case characters. Using 0059_cipher.txt
    ...
    for j in range(97, 123):
        for k in range(97, 123):
            ...
            for m in range(len(cipher)):
                key = [i, j, k][m % 3]
                decrypted_val = cipher[m] ^ key
                decrypted.append(decrypted_val)

            if max(decrypted) > 126 or min(decrypted) < 32:
                continue
