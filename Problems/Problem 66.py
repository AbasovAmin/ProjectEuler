...
def cf_sqrt_generator(D):
    """Yield infinite continued fraction terms of sqrt(D)."""
    a0 = int(math.isqrt(D))
    if a0 * a0 == D:
        return  # perfect square; no CF
...
def minimal_x(D):
    gen = cf_sqrt_generator(D)
    if gen is None:
        return None  # perfect square

    # Initial convergent state
    p_minus2, p_minus1 = 0, 1
    q_minus2, q_minus1 = 1, 0
